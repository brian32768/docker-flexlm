...
class ReadLmutil(object):
    ...
    def __init__(self, lmutil, test_mode=False):
        """
        Arguments:
    ...
    def get_fp(self):
        ...
        """
        if self.test_mode:
            test_file = self.lmutil    
            fp = open(test_file, 'r', encoding='utf-8')
        else:
            # Create a pipe to talk to lmutil
            print(self.lmutil)
            p = subprocess.Popen(self.lmutil, stdout=subprocess.PIPE, bufsize=1)
            fp = p.stdout
...
#    for e in os.environ: print(e, os.environ.get(e))
    lmutil = ReadLmutil("lmstat.txt", test_mode=True)
    try:
